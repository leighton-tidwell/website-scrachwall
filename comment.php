...
$ranname = "".get_random_string($name, 12).".".$extension;
    if (file_exists("upload/" . $ranname))
      ...
      "upload/" . $ranname);
	$query = mysql_query("SELECT * FROM `pictures` WHERE `id`='".mysql_real_escape_string($_GET['id'])."'");
	$fetch = mysql_fetch_array($query);
	$fintime = $fetch['deletetime'];
	mysql_query("INSERT INTO `comments` (`picture`,`main`,`time`,`deletetime`) VALUES('".mysql_real_escape_string($ranname)."','".mysql_real_escape_string($_GET['id'])."','".time()."','".$fintime."')");
